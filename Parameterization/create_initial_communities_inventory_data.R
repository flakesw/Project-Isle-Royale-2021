...
inventory_shape <- sf::st_read("./inventory_data/1 isrogisdata/isrogisdata/isroveg/isroveg.shp")
plot(sf::st_geometry(inventory_shape))
table(inventory_shape$TNC)
...
comm_mat <- fia_trees %>%
  ...
  dplyr::left_join(select(sp_ref, c("SPCD", "SPECIES_SYMBOL"))) %>%
  dplyr::rename(Plant.Symbol = SPECIES_SYMBOL) %>%
  group_by(PLT_CN, Plant.Symbol) %>%
  ...
  mutate(plot_sum = across(.cols = -1, .fns = ~ sum(.x)))
...
#-------------------------------------------------------------------------------
# compare FIA and ISRO plots

library("vegan")

isro_distmat <- vegdist(comm_mat_isro[, -1], method = "bray")
pca_data <- (comm_mat_isro[, -1])
pca_data <- sqrt(pca_data)
pca <- princomp(pca_data)
plot(pca)
biplot(pca, scale = 0.5)
biplot(pca, choice = c(1,3), scale = 0.5)

comm_mat
test <- bind_rows(comm_mat_isro[1, -1], comm_mat[3, -1]) %>%
