...
soilc_init <- rast("./Models/landis_test/mc_test - browse - linear - annual - subset/NECN/SOMTC-5.img")
plot(soilc_init)
hist(values(soilc_init)[values(soilc_init) > 0])
soilc1 <- rast("./Models/landis_test/mc_test - browse - linear - annual - subset/NECN/SOMTC-80.img")
plot(soilc1)
...
mean(values(soilc_change), na.rm = TRUE)
...
#what cells have crazy soils?
which(values(soilc_change) > 8000)


#exploring some explanatory variables-------------------------------------------
ic_map <- rast("./Models/LANDIS Inputs/input rasters hodgson/initial_communities_inv.tif")
ic_list <- read.csv("./Models/LANDIS inputs/NECN files/initial_communities_inv.csv")
table(values(ic_map)[values(clamped_ratio_change) > 0.99])
mapcodes <- unique(values(ic_map)[values(clamped_ratio_change) > 0.99]) %>% `[`(!is.na(.))
ic_list_reduced <- ic_list[ic_list$MapCode %in% mapcodes, ]
ic_list[ic_list$MapCode == 37, ]
boxplot(values(soilc_change) ~ values(ic_map))



soilm <- rast("./Models/landis_test/mc_test - browse - linear - annual - subset/NECN/SoilWater-55.img")
plot(soilm)
plot(values(soilc_change) ~ values(soilm))
...
wilt <- rast("./Models/LANDIS Inputs/input rasters hodgson/wilt_point.tif")
fc <- rast("./Models/LANDIS Inputs/input rasters hodgson/field_capacity.tif") 

stormflow <- rast("./Models/LANDIS Inputs/input rasters hodgson/stormflow.tif")
baseflow <- rast("./Models/LANDIS Inputs/input rasters hodgson/baseflow.tif") 
...
biomass_init <- rast("./Models/landis_test/mc_test - browse - linear - annual - subset/biomass/bio2-TotalBiomass-0.img")
NAflag(biomass_init) <- 0
plot(biomass_init)
hist(biomass_init)
plot(values(soilc_change) ~ values(biomass_init))
biomass_change <- rast("./Models/landis_test/mc_test - browse - linear - annual - subset/biomass/bio2-TotalBiomass-80.img") - biomass_init
plot(biomass_change)
plot(values(soilc_change) ~ values(biomass_change))
plot(soilc_change)

wind <- rast("./Models/landis_test/mc_test - linear/wind/severity-10.img")
plot(wind)

lai <- rast("./Models/landis_test/mc_test - one_cell/NECN/LAI-100.img")
plot(lai)
hist(values(lai)[values(lai)>0])
mean(values(lai)[values(lai)>0])

anpp <- rast("./Models/landis_test/mc_test - browse - linear - annual - subset/NECN/AG_NPP-40.img")
plot(anpp)
hist(values(anpp)[values(anpp)>0])
mean(values(anpp)[values(anpp)>0])
plot(values(anpp) ~ values(lai))


plot(values(soilc_change) ~ values(anpp))

alder_biomass <- rast("./Models/landis_test/mc_test - linear/biomass/bio2-ALIN2-0.img")

anerb <-  rast("./Models/landis_test/mc_test - browse - linear - annual - subset/NECN/AnaerobicEffect-50.img")
plot(anerb)
...
plot(values(soilm)[values(clamped_ratio_change) > 0.99] ~ values(soilc_change)[values(clamped_ratio_change) > 0.99])
...
#mostly well-drained, dry, sandy soil with a lot of dead trees

#som1Csurf
som1surf_init <- rast("./Models/landis_test/mc_test - browse - linear - annual - subset/NECN-Initial-Conditions/SOM1Csurface-5.img")
plot(som1surf_init)
som1surf_80 <- rast("./Models/landis_test/mc_test - browse - linear - annual - subset/NECN-Initial-Conditions/SOM1Csurface-80.img")
plot(som1surf_80)
...
##som1Csoil
som1soil_init <- rast("./Models/landis_test/mc_test - browse - linear - annual - subset/NECN-Initial-Conditions/SOM1Csoil-20.img")
plot(som1soil_init)
som1soil_80 <- rast("./Models/landis_test/mc_test - browse - linear - annual - subset/NECN-Initial-Conditions/SOM1Csoil-80.img")
plot(som1soil_80)
...
#SOM2----------------------
som2_init <- rast("./Models/landis_test/mc_test - browse - linear - annual - subset/NECN-Initial-Conditions/SOM2C-20.img")
plot(som2_init)
som2_80 <- rast("./Models/landis_test/mc_test - browse - linear - annual - subset/NECN-Initial-Conditions/SOM2C-80.img")
plot(som2_80)
...
anpp_5 <- rast("./Models/landis_test/mc_test/NECN/AG_NPP-30.img")
plot(anpp_5)
...
biomass_init <- rast("./Models/landis_test/mc_test - browse - linear/biomass/bio2-TotalBiomass-0.img")
plot(biomass_init)
hist(values(biomass_init)[values(biomass_init) > 0])
mean(values(biomass_init)[values(biomass_init) > 0])

biomass1 <- rast("./Models/landis_test/mc_test - browse - linear/biomass/bio2-TotalBiomass-80.img")
plot(biomass1)
...
mean(values(biomass_change), na.rm = TRUE)
sd(values(biomass_change), na.rm = TRUE)
...
zoom(comm_map_test)
click(comm_map_test)
table(values(comm_map_test))

plot(values(biomass_init) ~ values(soilc_init))
abline(0,1)

plot(values(biomass_init)[which(values(biomass_change) < -20000)] ~ values(soilc_init)[which(values(biomass_change) < -20000)])
summary(lm(values(biomass_init) ~ values(soilc_init)))
summary(lm(values(biomass_init)[which(values(biomass_change) < -20000)] ~ values(soilc_init)[which(values(biomass_change) < -20000)]))

### What sort of sites experienced greatest increases in biomass?
comm_output_init <- read_csv("./Models/LANDIS inputs/NECN files/initial_communities_inv.csv")
comm_map_init <- rast("./Models/LANDIS inputs/input rasters/initial_communities_inv.tif")

sites_high_biomass_init <- values(comm_map_init)[which(values(biomass_change) > 15000)]
comm_high_biomass_init <- comm_output_init[comm_output_init$MapCode %in% sites_high_biomass_init, ]
sites_high_biomass_end <- values(comm_map_end)[which(values(biomass_change) > 15000)]
...
test <- comm_high_biomass_init %>%
  ...
  summarise(total_biomass_init = sum(CohortBiomass))
test2 <- comm_high_biomass %>%
  group_by(SpeciesName) %>%
  summarise(total_biomass_end = sum(CohortBiomass))

join <- left_join(test2, test, by = "SpeciesName")
join$change <- join$total_biomass_end - join$total_biomass_init



#site to investigate
sites <- which(values(biomass_change) < -20000)
...
test <- biomass_change
values(test) <- 0
values(test)[values(comm_map_init) == 100] <- values(biomass_change)[values(comm_map_init) == 100]
NAflag(test) <- 0
plot(test)
...
alder_stack <- list.files("./Models/landis_test/mc_test - linear/biomass",
                          full.names = TRUE) %>%
  `[`(grepl("ALIN", .)) %>%
  rast()
...
plot(alder_stack[[1]])
plot(alder_stack[[9]])
plot(alder_stack[[9]] -alder_stack[[2]])
...
##
soil_moisture_stack <-  list.files("./Models/landis_test/mc_test - linear/necn",
                                   full.names = TRUE) %>%
...
#------------------------------------------------------------------------------
# Compare models --------------------------------------------------------------
biomass_nobrowse <- rast("./Models/landis_test/mc_test - linear/biomass/bio2-TotalBiomass-80.img")
plot(biomass_nobrowse)
hist(values(biomass_nobrowse)[values(biomass_nobrowse) > 0])

biomass_browse <- rast("./Models/landis_test/mc_test - browse - linear/biomass/bio2-TotalBiomass-80.img")
plot(biomass_browse)
zoom(biomass_browse)
...
zoom(diff)
NAflag(diff) <- 0
mean(values(diff), na.rm = TRUE) #average "moose effect" on biomass
...
biomass_nobrowse <- rast("./Models/landis_test/mc_test - linear/NECN/SOMTC-5.img")
plot(biomass_nobrowse)
hist(values(biomass_nobrowse)[values(biomass_nobrowse) > 0])

biomass_browse <- rast("./Models/landis_test/mc_test - browse - linear/NECN/SOMTC-80.img")
plot(biomass_browse)
zoom(biomass_browse)
hist(values(biomass_browse)[values(biomass_browse) > 0])
